  ...
  group('parseOptions', () {
    ...
    group('debugFillProperties', () {
      ...
      test('throws if not bool', () {
        expect(
          () => parseBuilderOptions(const BuilderOptions(
              <String, dynamic>{'debugFillProperties': 42})),
          throwsArgumentError,
        );
      });
      test('parses valid value', () {
        expect(
          parseBuilderOptions(const BuilderOptions(
                  <String, dynamic>{'debugFillProperties': true}))
              .debugFillProperties,
          true,
        );
        expect(
          parseBuilderOptions(const BuilderOptions(
                  <String, dynamic>{'debugFillProperties': false}))
              .debugFillProperties,
          false,
        );
      });
    ...
    group('widgetType', () {
      ...
      test('throws if string but not valid', () {
        expect(
          () => parseBuilderOptions(
              const BuilderOptions(<String, dynamic>{'widgetType': 'foo'})),
          throwsArgumentError,
        );
      });
      test('throws if not string', () {
        expect(
          () => parseBuilderOptions(
              const BuilderOptions(<String, dynamic>{'widgetType': 42})),
          throwsArgumentError,
        );
      });
      test('parses valid value', () {
        expect(
          parseBuilderOptions(
                  const BuilderOptions(<String, dynamic>{'widgetType': 'hook'}))
              .widgetType,
          FunctionalWidgetType.hook,
        );
        expect(
          parseBuilderOptions(const BuilderOptions(
              <String, dynamic>{'widgetType': 'stateless'})).widgetType,
